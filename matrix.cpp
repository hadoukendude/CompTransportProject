...
void initializeXS(struct Problem& prob, struct Geometry& geom, struct MatrixData& matrix_data)
{
    ...
    for (int index = 0; index < n; index++) // meshpoint/subinterval number
    {
        i = 0;
        ...
        if (prob.problemType == ProblemType::fixedsource)
            matrix_data.G[index] = prob.source[i]*matrix_data.D[i];
...
{
    int n = collProbMat.rows();
    matData.H = Eigen::MatrixXd::Zero(n,n);
